...
export const FOREST_MAP_DATA = {
    ...
    spawnArea: (grid, mapCtx) => ({
        position: {
            ...
            y: grid.position.y + mapCtx.properties.tileSize * 15
        },
    ...
    collisionBlocks: (bg, mapCtx) => [
        { position: { x: 0, y: bg.height - 190 }, width: bg.width, height: mapCtx.properties.tileSize },
    ...
    interactableAreas: (bg, grid, mapCtx) => [
        {
            finish: true,
            position: {
                x: grid.position.x + mapCtx.properties.tileSize * 40,
                y: grid.position.y + mapCtx.properties.tileSize * 15
            },
