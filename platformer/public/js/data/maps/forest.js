function createForest(){
    ...
    const scaleForest = 1.8 * canvas.width / 1024;
    const playerScaleForest = .1;
    const allCollisionBlocksForest = [];
    const tileSize = 240 * playerScaleForest;
    
    const startAreaForest = {
        position: {x: gridForest.position.x + tileSize*1, y: gridForest.position.y + tileSize*11},
        width: tileSize * 5,
        height: tileSize * 3
    };
    const finishAreaForest = {
        position: {x: gridForest.position.x + tileSize*31, y: gridForest.position.y + tileSize*11},
        width: tileSize * 5,
        height: tileSize * 3
    };
    ...
    return [
        ...
        startAreaForest, finishAreaForest, allCollisionBlocksForest,
